...
export const POST = async ({ params, locals }: RequestEvent) => {
	try {
		...
		const success = await deploymentService.stopProject(project.id);
		const newStatus = success ? 'stopped' : project.status;

		if (success) {
			await dbUtils.updateProjectStatus(project.id, 'stopped');
			await dbUtils.addDeploymentLog(project.id, 'info', 'Project stopped successfully');
		} else {
